...
export class ResendMailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (apiKey) {
      ...
      if (this.enableDebugLogs) {
        this.logger.log('✅ ResendMailService inicializado correctamente');
      }
    } else {
      this.logger.warn(
        '⚠️ RESEND_API_KEY no configurada. El servicio de email alternativo no estará disponible.',
      );
    }
  }
  ...
  async sendPasswordResetEmail(email: string, resetToken: string) {
    if (!this.resend) {
      throw new Error('Resend API no está configurada (falta RESEND_API_KEY)');
    }
    ...
    if (this.enableDebugLogs) {
      this.logger.log(`📧 [Resend] Enviando email a: ${email}`);
      this.logger.log(`🔗 [Resend] URL de reset: ${resetUrl}`);
    }
    ...
    try {
      ...
      if (this.enableDebugLogs) {
        this.logger.log(
          `✅ [Resend] Email enviado exitosamente. ID: ${data.data?.id}`,
        );
      }

      return { success: true, messageId: data.data?.id, info: data };
    } catch (error) {
      this.logger.error(
        `❌ [Resend] Error enviando email: ${error.message}`,
        error.stack,
      );
      throw error;
    }
